...
if (window.Telegram && Telegram.WebApp) {
  ...
  if (tgUser) {
    document.getElementById("telegramUserBox").innerHTML =
      `👤 ${tgUser.first_name} (@${tgUser.username || "no_username"})`;
  }
}
...
function loop() {
  ...
  pipes.forEach((p,i)=>{
    ...
    if(bird.x+30>p.x && bird.x-30<p.x+p.w &&
      (bird.y-22<p.top || bird.y+22>p.bottom)){
      endGame();
    }
...
async function openLeaderboard(){
  ...
  arr.slice(0,30).forEach((p,i)=>{
    html += `<div>${i+1}. ${p.name} (@${p.username}) - ${p.score}</div>`;
  });
